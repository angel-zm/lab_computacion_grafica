...
void DoMovement()
{
	...
	// Front legs control: U = subir (flexionar), I = bajar (extender)
	if (keys[GLFW_KEY_U])
	{
		FLegs += 0.5f;
	...
	// Back legs control: O = subir (flexionar), P = bajar (extender)
	if (keys[GLFW_KEY_O])
	{
		RLegs += 0.5f;
	...
	// Tail control: C = mover en una dirección, V = en la otra
	if (keys[GLFW_KEY_C])
	{
		tail += 0.5f;
		if (tail > 45.0f) tail = 45.0f;
	}
	if (keys[GLFW_KEY_V])
	{
		tail -= 0.5f;
